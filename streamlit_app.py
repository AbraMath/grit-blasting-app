...
with col1:
    ...
    diameter = st.number_input("Turntable Diameter (in)", 1.0, 100.0, 36.0)
    blast_time = st.slider("Blast Duration (sec)", 5, 60, 30)
with col2:
    nozzle_rpm = st.slider("Nozzle Assembly RPM", 1, 60, 40)
    num_nozzles = st.slider("Number of Nozzles", 1, 12, 6)
    nozzle_radius = diameter / 4  # nozzle rotation radius

radius = diameter / 2  # Turntable radius

# --- FRAME SELECTION ---
frame = st.slider("Animation Frame", 0, 60, 0)

# --- CALCULATIONS ---
turntable_angle = 2 * np.pi * (turntable_rpm / 60) * (frame / 30)
nozzle_angle = -2 * np.pi * (nozzle_rpm / 60) * (frame / 30)

# Nozzle positions in nozzle assembly coordinates
angles = np.linspace(0, 2 * np.pi, num_nozzles, endpoint=False)
x = nozzle_radius * np.cos(angles + nozzle_angle)
y = nozzle_radius * np.sin(angles + nozzle_angle)

# Rotate entire assembly with turntable
